    ...
    const handleKeyDown = useCallback(
      (e: KeyboardEvent) => {
        ...
        if (e.key === 'ArrowUp') {
          ...
          if (row < maxRows - 1) nextAddr = `${String.fromCharCode(65 + col)}${row + 2}`
        } else if (e.key === 'ArrowLeft') {
          if (col > 0) nextAddr = `${String.fromCharCode(65 + col - 1)}${row + 1}`
        } else if (e.key === 'ArrowRight') {
          if (col < maxCols - 1) nextAddr = `${String.fromCharCode(65 + col + 1)}${row + 1}`
        } else if (e.key === 'Tab') {
          if (e.shiftKey) {
            if (col > 0) nextAddr = `${String.fromCharCode(65 + col - 1)}${row + 1}`
            else if (row > 0) nextAddr = `${String.fromCharCode(65 + maxCols - 1)}${row}`
          } else {
            if (col < maxCols - 1) nextAddr = `${String.fromCharCode(65 + col + 1)}${row + 1}`
            else if (row < maxRows - 1) nextAddr = `${String.fromCharCode(65)}${row + 2}`
          }
        } else if (e.key === 'Home') {
      ...
      [sheet, selectedCell, isEditing, setSelectedCell, updateCell, activeContent]
    )
              ...
              <tr key={row}>
                ...
                {Array.from(
                  ...
                  (_, col) => {
                    ...
                    return (
                      <td className="border-collapse border-gray-300 border" key={col}>
